...
void sortedInsert(struct node *newNode){
	...
	int nodeAdded = 1;           // task is intialize with 1, if newNode is added to the list -
	...
	}else if(current == head && current->task->priority > newNode->task->priority){
		...
		printf("Task made as new head ");
                ...
                while(nodeAdded != 0 && current->next != NULL && current->task->priority <= newNode->task->priority){
			//if priority is the same as the newNode's, add newNode to the end of the same priority list
			if(current->task->priority == newNode->task->priority){
...
void add(char *name, int priority, int burst){
        ...
        taskNode = malloc(sizeof(taskNode) * (4));
        taskNode->name = name;
        taskNode->tid = 0;
		...
		printf("If priority are the same, algorithm will follow a first come, first served basis\n\n");
                printf("first task inserted, assigned as head");
		task_list->next = head;
                head = task_list;
        }else{
...
void schedule(){
        printf("\n\n");
	printf("List shown in the order of priority\n");
///////////////////////////////////////
